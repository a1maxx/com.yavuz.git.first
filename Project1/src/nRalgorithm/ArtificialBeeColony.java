...
public class ArtificialBeeColony {
	...
	public ArtificialBeeColony(int n) {
		MAX_LENGTH = n;
		NP = 60; // pop size 20 to 40 or even 100
		FOOD_NUMBER = NP / 2;
		LIMIT = 5;
		MAX_EPOCH = 1000;
		MIN_SHUFFLE = 1;
		...
		while (!done) {
			if (epoch < MAX_EPOCH) {
				...
				for (int i = 0; i < MAX_LENGTH; i++) {
					System.out.printf("%.3f\t", gBest.getNectar(i));
				}
				System.out.println(gBest.getConflicts());
...
//       
	...
	public void sendToWork(Honey currentBee, Honey neighborBee) {
		...
		newValue = (tempValue
				+ (tempValue - neighborBee.getNectar(parameterToChange)) * (rand.nextDouble() - 0.5) * 0.5);
...
//       tempIndex = currentBee.getIndex(tempValue);
...
//         currentBee.setNectar(tempIndex, newValue);
	...
	/*
	 * Finds food sources which have been abandoned/reached the limit. Scout bees
	 * will generate a totally random solution from the existing and it will also
	 * reset its trials back to zero.
	 *
	 */
